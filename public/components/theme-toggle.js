...
class ThemeToggle extends HTMLElement {
  ...
  disconnectedCallback() {
    this.shadowRoot.querySelector('button')?.removeEventListener('click', this.#handleToggleClick);
  ...
  #applyTheme() {
    ...
    if (themeColorMeta) {
      const darkThemeColor = getComputedStyle(document.documentElement)
        .getPropertyValue('--color-background').trim() || '#121212';
  ...
  setTheme(theme) {
    if (Object.values(this.#themes).includes(theme)) {
      ...
      console.warn(`Invalid theme: ${theme}. Must be one of: ${Object.values(this.#themes).join(', ')}`);
    }
  }
}
